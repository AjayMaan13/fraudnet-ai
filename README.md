...
🟢 `clean` (0–20) · 🟡 `watch` (21–50) · 🟠 `suspicious` (51–75) · 🔴 `fraud` (76–100)
...
| ⚙️ Backend | Python 3.11, FastAPI, WebSocket, uvicorn |
| 🕸️ Graph Engine | NetworkX — cycle detection, Louvain, PageRank, sliding-window burst/fanout |
...
| 🗄️ Database | SQLite _(IBM Db2 connection removed to keep the app running)_ |
...
Click **⚙️ Reconfigure** in the dashboard to open the simulation configurator:

| 🎛️ Control | 📦 What it generates |
|---|---|
...
**Presets:** ⚡ Quick · 🔵 Default · 🔥 Heavy · 🌪️ Chaos
...
```
GET  /graph            — 🕸️  full graph (nodes + edges + risk scores)
...
POST /db2/load         — 🗄️  reload from Db2/SQLite, broadcast reset to all clients
...
- **▶️ Start:** `python -m uvicorn backend.main:app --host 0.0.0.0 --port $PORT`
